            ...
            if (int.TryParse(Console.ReadLine(), out int choice))
            {
                switch (choice)
                {
                    case 1:
                        ...
                        {
                            account.CurrentState.UseInternet(account, megabytes);
                        }
                        else
                        {
                    ...
                    case 2:
                        ...
                        if (int.TryParse(Console.ReadLine(), out int minutes))
                        {
                            ...
                            account.CurrentState.MakeCall(account, minutes, number);
                        }
                        else
                        {
                            Console.WriteLine("Некоректне значення.");
                    ...
                    case 4:
                        ...
                        if (int.TryParse(Console.ReadLine(), out int data))
                        {
                            ...
                            if (int.TryParse(Console.ReadLine(), out int inputMinutes))
                            {
                                account.CurrentState.CheckPackage(account, data, inputMinutes);
                            }
                            else
                            {
